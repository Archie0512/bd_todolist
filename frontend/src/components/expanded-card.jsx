...
function ExpandedCard(props) {
  ...
  function uploadImage(file) {
    // Image upload disabled in Supabase mode
  ...
  onMount(() => {
    ...
    setEditor(newEditor);
    try {
      const editorRoot = editorContainerRef.childNodes[0];
      if (!editorRoot) return;
      const toolbar = editorRoot.childNodes[1];
      if (!toolbar) return;
      const toolbarGroup = toolbar.childNodes[0];
      if (!toolbarGroup) return;
      const secondGroup = toolbarGroup.childNodes[1];
      if (!secondGroup) return;
      const firstItem = secondGroup.childNodes[0];
      if (!firstItem) return;
      const toolbarEndGroupNodes = [...firstItem.childNodes];
      const modeBtns = toolbarEndGroupNodes.filter((node) => node.title);
      setModeBtns(modeBtns);
    } catch (e) {
      console.warn("Could not extract toolbar buttons:", e);
    }
  });
  ...
  createEffect(() => {
    ...
    const editorRoot = editorContainerRef.childNodes[0];
    if (editorRoot && editorRoot.childNodes[2]) {
      editorRoot.childNodes[2].focus();
    }
  });
            ...
            <div class="dialog__tags-and-due-date">
              <div class="dialog__tags">
                <For each={availableTags()}>
                  {(tag) => {
                    const isChecked = (props.tags || []).some(
                      (t) => t.name === tag.name
                    );
                    return (
                      <label
                        class={"tag-checkbox" + (isChecked ? " tag-checkbox--active" : "")}
                        style={{ "background-color": isChecked ? tag.backgroundColor : "transparent", "border-color": tag.backgroundColor }}
                      >
                        <input
                          type="checkbox"
                          checked={isChecked}
                          ...
                          onChange={(e) => {
                            if (e.target.checked) {
                              const actualContent = editor().content;
                              const newContent = addTagToContent(actualContent, tag.name);
                              props.onContentChange(newContent);
                              editor().content = newContent;
                            } else {
                              deleteTag(tag.name);
                ...
                <input
                  type="text"
                  class="tag-new-input"
                  placeholder={props.t()('expandedCard.newTagPlaceholder')}
                  onKeyDown={(e) => {
                    if (e.key === "Enter" && e.target.value.trim()) {
                      e.preventDefault();
                      const tagName = e.target.value.trim();
                      const actualContent = editor().content;
                      const newContent = addTagToContent(actualContent, tagName);
                      props.onContentChange(newContent);
                      editor().content = newContent;
                      e.target.value = "";
                    }
